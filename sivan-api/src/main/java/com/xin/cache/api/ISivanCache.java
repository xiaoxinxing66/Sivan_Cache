 ...
 * @author sivan
 */
public interface ISivanCache<K, V> extends Map<K, V> {
     ...
     * （1）惰性删除。
     * 在执行下面的方法时，如果过期则进行删除。
     * {@link ISivanCache#get(Object)} 获取
     * {@link ISivanCache#values()} 获取所有值
     * {@link ISivanCache#entrySet()} 获取所有明细
     *
     * 【数据的不一致性】
     * 调用其他方法，可能得到的不是使用者的预期结果，因为此时的 expire 信息可能没有被及时更新。
     * 比如
     * {@link ISivanCache#isEmpty()} 是否为空
     * {@link ISivanCache#size()} 当前大小
     ...
     * @return 处理类实现
     */
    ISivanCacheExpire<K,V> expire();

    /**
     * 删除监听类列表
     * @return 监听器列表
     */
    List<ISivanCacheRemoveListener<K,V>> removeListeners();

    /**
     * 慢日志监听类列表
     * @return 监听器列表
     */
    List<ISivanCacheSlowListener> slowListeners();

    /**
     * 加载信息
     * @return 加载信息
     */
    ISivanCacheLoad<K,V> load();

    /**
     * 持久化类
     * @return 持久化类
     */
    ISivanCachePersist<K,V> persist();

    /**
     * 淘汰策略
     * @return 淘汰
     */
    ISivanCacheEvict<K,V> evict();
