package com.xin.cache.support.struct.lru.impl;
...
import com.xin.cache.model.CircleListNode;
...
public class LruMapCircleList<K,V> implements ILruMap<K,V> {
     ...
     */
    @Override
    public ISivanCacheEntry<K, V> removeEldest() {
        //fast-fail
        if(isEmpty()) {
        ...
        while (node.next() != this.head) {
            ...
            if(!node.accessFlag()) {
                ...
                this.removeKey(key);
                return SivanCacheEntry.of(key, node.value());
            } else {
        ...
        return SivanCacheEntry.of(firstNode.key(), firstNode.value());
    }
